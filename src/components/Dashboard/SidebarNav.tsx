...
const footerNavItems: NavItem[] = [
  { id: 'help1', label: 'Help', icon: HelpCircle, href: '#' }, // Changed ID to be unique
  { id: 'settings', label: 'Settings', icon: Settings, href: '#' },
  { id: 'help2', label: 'Help', icon: HelpCircle, href: '#' }, // Changed ID to be unique
];
          ...
          {mainNavItems.map((item) => (
            <li key={item.id}>
              ...
              >
                ...
                {item.badge && (
                  <span className="ml-auto rounded-full bg-accentRed px-2 py-0.5 text-xs text-white">
                    {item.badge}
                  </span>
                )}
              </a>
